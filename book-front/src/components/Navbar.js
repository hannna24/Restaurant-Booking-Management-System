    ...
    <div className="custom-navbar">
      ...
      <nav className="navbar navbar-expand-lg bg-light">
        <div className="container">
          ...
          <a className="navbar-brand fw-bold text-dark" href="#" style={{ fontFamily: 'Pacifico, cursive', fontSize: '1.7rem' }}>
            Bistro Bliss
          </a>
          ...
          <div className="collapse navbar-collapse justify-content-end" id="navbarNav">
            <ul className="navbar-nav me-3">
              ...
              <li className="nav-item"><Link className="nav-link" to="/blog">Pages</Link></li>
              <li className="nav-item"><Link className="nav-link" to="/contact">Contact</Link></li>
            </ul>
            <Link className="btn btn-outline-dark rounded-pill me-2" to="/book">Book A Table</Link>

            {isLoggedIn ? (
              <button className="btn btn-danger rounded-pill" onClick={handleLogout}>Logout</button>
            ) : (
              <>
                <Link className="btn btn-outline-primary rounded-pill me-2" to="/login">Login</Link>
                <Link className="btn btn-outline-success rounded-pill" to="/register">Register</Link>
              </>
            )}
          </div>
        </div>
